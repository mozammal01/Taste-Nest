...
export default function FoodMenu() {
  ...
  return (
    <div className="w-full flex flex-col items-center gap-6">
          ...
          return (
            <div className="relative" key={cat.id}>
              <div
                ...
                className={cn(
                  "flex flex-col justify-center items-center gap-2 border shadow-sm rounded-xl transition w-[190px] h-[190px] cursor-pointer p-8",
              ...
              {active === cat.id && <Image src={mask} alt="mask" width={168} height={40} className="absolute top-[169px] left-3" />}
            </div>
          );
        })}
      </div>
      ...
      {/* Content */}
      <div className="flex flex-col md:flex-row w-full max-w-4xl rounded-2xl h-[300px] my-10 mt-20">
        <div className="relative">
          <Image src={foods[active].image} alt={foods[active].title} height={300} width={300} className="object-cover rounded-2xl" />
        </div>
        <CardContent className="p-6 min-w-[580px] bg-slate-50 rounded-2xl">
